...
if (show_alch) {
	...
	if (nx >= 0 and nx < alch_slots_width-1 and ny >= 0 and ny < alch_slots_height and i_mousey > 0 and i_mousex > 0) ||
	...
	if ((output_mousex >= 0 and output_mousex <= 15*scale) and (output_mousey >= 0 and output_mousey <= 15*scale)) {
		...
		selected_slot = min(alchemy_slots - 2, m_slotx + (m_sloty * alch_slots_width));	
